...
const authenticate = (req, res, next) => {
  ...
  if (req.method === 'POST' && req.originalUrl.endsWith('/api/auth/logout')) return next();
  ...
  jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
    ...
    if (!decoded || !decoded.id || !decoded.role) return res.status(401).json({ error: "Token inválido: datos incompletos." });
